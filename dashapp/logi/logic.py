...
class Proposition:
    ...
    def is_equivalent(self, other):
        ...
        return all(
            self.evaluate(**r) == other.evaluate(**r) for r in truth_table_rows(self.variables() | other.variables()))
...
class ArgumentForm:
    ...
    def __str__(self):
        return ((', '.join(str(c) for c in self.premises) + ', ') if self.premises else '') + 'conclusion = ' + str(
            self.conclusion)
    ...
    def print_truth_table(self):
        ...
        print('{0}  #  {1: ^{2}}  #  {3: ^{4}}'.format(' ' * len(vars_header), 'premises', len(premises_header),
                                                       'conclusion', len(conclusion_string)))
        ...
        for r in rows:
            ...
            premise_cols = '   '.join(
                '{0: ^{1}}'.format(str(v) + star, len(k)) for (k, v) in zip(premise_strings, premise_values))
    ...
    def is_valid(self):
        vars = (frozenset.union(
            *[c.variables() for c in self.premises]) if self.premises else frozenset()) | self.conclusion.variables()
        return all(self.conclusion.evaluate(**r) for r in truth_table_rows(vars) if
                   all(c.evaluate(**r) for c in self.premises))

    def substitute(self, e1, e2):
        return ArgumentForm(*[c.substitute(e1, e2) for c in self.premises],
                            conclusion=self.conclusion.substitute(e1, e2))
#


if __name__ == '__main__':
    P, Q, R, S = vars('P', 'Q', 'R', 'S')
    arg = ArgumentForm( *(P, P >> Q), conclusion=Q)
    arg.is_valid()
    ...
    prems = P & ~P
    prems.is_contradiction()

    prems = P & (P >> Q)
    arg = ArgumentForm(prems, conclusion=P)
    arg.is_valid()
    ...
    arg = ArgumentForm(*(eval(p, vars) for p in premises), conclusion=~P)
    print(arg.is_valid())
